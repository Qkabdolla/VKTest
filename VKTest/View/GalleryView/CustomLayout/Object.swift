...
final class SquareMosaicObject {
    ...
    init?(_ numberOfItemsInSections: [Int], _ dataSource: DataSource?, _ size: CGSize) {
        guard let dataSource = dataSource else {
            return nil
        }
        let attributesAndContentSize = getAttributesAndContentSize(numberOfItemsInSections, dataSource, size)
...
private func getAttributesAndContentSize(_ numberOfItemsInSections: [Int], _ dataSource: DataSource, _ size: CGSize) -> (attributes: Attributes, contentSize: CGFloat) {
    ...
    for (rows, section) in Array(0 ..< numberOfItemsInSections.count).map({ (numberOfItemsInSections[$0], $0) }) {
        ...
        if let (attributes, separator) = getAttributesSupplementary(.header, dataSource, origin, rows: rows, section, size) {
        ...
        if let (attributes, separator) = getAttributesCells(pattern, origin, rows, section, size) {
            if let separator = separator {
                origin += separator
            }
        ...
        if let (attributes, separator) = getAttributesSupplementary(.footer, dataSource, origin, rows: rows, section, size) {
            if let separator = separator {
                origin += separator
            }
            attributesSupplementary.append(attributes)
        }
        if let (attributes, _) = getAttributesSupplementary(.backer, dataSource, origin, section, sectionOrigin: sectionOrigin, size) {
            attributesSupplementary.append(attributes)
        }
    }
...
private func getAttributesCells(_ pattern: Pattern, _ origin: CGFloat, _ rows: Int, _ section: Int, _ size: CGSize) -> (attributes: [UICollectionViewLayoutAttributes], separator: CGFloat?)? {
...
private func getAttributesSupplementary(_ kind: SupplementaryKind, _ dataSource: DataSource, _ origin: CGFloat, rows: Int = 0, _ section: Int, sectionOrigin: CGFloat = 0, _ size: CGSize) -> (attributes: UICollectionViewLayoutAttributes, separator: CGFloat?)? {
    switch kind {
    case .backer:
